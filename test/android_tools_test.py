...
# 应用图标路径
icon_path =  project_path + "\\icons\\android.png"
# 安装日志路径
install_log_path = project_path + "\\results\\install_result.txt"
# 卸载日志路径
uninstall_log_path = project_path + "\\results\\uninstall_result.txt"
# monkey执行测试日志路径
monkey_log_path = project_path + "\\results\\monkey_result.txt"
...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def mainUI(self, MainWindow):
        ...
        self.installBtn_2.clicked.connect(self.view_install_log)
        self.uninstallBtn.clicked.connect(self.uninstall_app)
        self.installBtn_3.clicked.connect(self.view_uninstall_log)
    ...
    def install_app(self):
        try:
            if self.printAppRoute.text().strip():
                ...
                f = open(install_log_path, 'w')
                data = subprocess.Popen(cmd, stdout=f.fileno(),stderr=f.fileno(), shell=True)
    ...
    def view_install_log(self):
        QMessageBox.information(self, "日志查看", "请到results目录查看install_result.txt文件", QMessageBox.NoButton)
    ...
    def uninstall_app(self):
        try:
            if self.appName_1.text().strip():
                ...
                fb = open(uninstall_log_path, 'w')
                data = subprocess.Popen(cmd, stdout=fb.fileno(), stderr=fb.fileno(), shell=True)
                QMessageBox.information(self, "提示", "请5秒后查看安装结果", QMessageBox.NoButton)
                data.kill()
                fb.flush()
                fb.close()
    ...
    # 查看卸载app日志
    def view_uninstall_log(self):
        QMessageBox.information(self, "日志查看", "请到results目录查看uninstall_result.txt文件", QMessageBox.NoButton)
    ...
    def judge_sum(self):
        try:
            sum_1 = float(self.touchInput.text())+float(self.motionInput.text())+float(self.pinchInput.text())+\
                    float(self.screenInput.text())+float(self.navInput.text())+float(self.majornavInput.text())+\
                    float(self.trackballInput.text())+float(self.sysykeysInput.text())+float(self.anyeventInput.text())+\
                    float(self.keyboardInput.text())+float(self.appswitchInput.text())
            print(sum_1)
            if sum_1 == 100:
                return True
        except:
    ...
    def run_command(self,cmd):
        ...
        try:
            #方式一
            fb = open(monkey_log_path, 'w')
            data = subprocess.Popen(cmd, stdout=fb.fileno(), stderr=fb.fileno(), shell=True)
            QMessageBox.information(self, "提示", "请查看日志信息确定执行结果", QMessageBox.NoButton)
            data.kill()
            fb.flush()
            fb.close()

            '''方式二
            cmd = cmd + ">"+monkey_log_path
            print(cmd)
            os.popen(cmd)
             QMessageBox.information(self, "提示", "请查看日志信息确定执行结果", QMessageBox.NoButton)
            '''
    ...
    # 查看monkey测试日志记录
    def view_log(self):
        QMessageBox.information(self, "日志查看", "请到results目录查看monkey_result.txt文件", QMessageBox.NoButton)
